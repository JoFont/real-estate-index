...
async function getPages() {
	...
	console.log(
		`${chalk.cyan("Number of Pages Fetched:")} ${chalk.cyan.bold(
			numberOfPages
		)} => ${chalk.green("SUCCESS")}`
	);
...
async function getPageResults(pageNum) {
	...
	console.log(
		`${chalk.cyan("Fetching results for page:")} ${chalk.cyan.bold(pageNum)}`
	);
	...
	articles.each((i, el) => {
		...
		if (features && features["cat_l3_name"]) {
			const title = $(el)
				.find(".title-cell > div > h3 > a > strong")
				.text();
			const imgUrl = $(el)
				.find("td > a.thumb > img")
				.attr("src");
			const propertyType = features["cat_l3_name"]
				? features["cat_l3_name"].split("-")[0].slice(0, -1)
				: null;
			const listingType =
				features["cat_l3_name"].split("-")[1] === "venda"
					? "buy"
					: "rent";
	...
	console.log(
		`${chalk.cyan("Fetching results for page:")} ${chalk.cyan.bold(
			pageNum
		)} => ${chalk.green("SUCCESS")} ---- ${chalk.yellow(
			"Total Number of Results: "
		)} ${chalk.yellow.italic(results.length)}`
	);
